...
public class EsperQueries{
    ...
    public static final String ALERTA_TRAFICO_MARINO =
        ...
        "count(*) as lecturas " +
        "FROM Sonido#sliding_time_window(5 min) " +
    ...
    public static final String ALERTA_TERREMOTO =
        ...
        "WHERE ( " +
        "SELECT count(*) " +
        "FROM Presion#time(2 min) p2 " +
        "WHERE (p2.pressure_dbar < 30 OR p2.pressure_dbar > 500) " +
        "AND Math.abs(p2.lat - p.lat) <= 2 " +
        "AND Math.abs(p2.lon - p.lon) <= 2 " +
        ") >= 2";

    // Devuelve todas las queries en orden
